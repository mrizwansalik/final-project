...
if($email==true){
		...
		header('location:Trainerslist.php');
	} 
...
if($_POST["submit"]){
	...
	$update= "UPDATE trainer SET fullName='$fullname', password='$password', contact='$contactno', address='$address' WHERE id = '$val'";
	$conn->exec($update);
		...
		<div class="container">
			<div class="row" style=" padding: 20px 0px 20px 0px;">
				<div class="col-md-6 offset-3 bg-light" style="border: 1px solid white; border-radius: 10px;">
					...
					<?php
						// $select = $conn->query("SELECT * FROM trainer where email='$email'");
						$select = $conn->query("SELECT * FROM trainer where id='$val'");
						while($val1 = $select->fetch()){
					?>
					<form method="post"  >
						<div class="form-group">
							<label for="fullname">Full Name:</label>
							<input type="text" name="fullname" class="form-control" required placeholder="Full Name" value="<?php echo $val1['fullName']; ?>">
						</div>
						<div class="form-group">
							<label for="password">Password:</label>
							<input type="text" name="password" class="form-control" required placeholder="password" value="<?php echo $val1['password']; ?>">
						</div>
						<div class="form-group">
							<label for="contact">Contact No:</label>
							<input type="number" name="contactno" class="form-control" required placeholder="contact" value="<?php echo $val1['contact']; ?>">
						</div>
						<div class="form-group">
							<label for="address">Address:</label>
							<input type="text" name="address" class="form-control" required placeholder="address" value="<?php echo $val1['address']; ?>">
						</div>
						<div class="form-group">
							<label for="experience">Experience:</label>
							<input type="text" name="experience" class="form-control" required placeholder="experience" value="<?php echo $val1['experience']; ?>">
						</div>
